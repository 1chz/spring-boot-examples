...
여기서 작업을 진행할 때 이미 큐에서 작업을 꺼내온 상태이므로, 에러가 발생한다면 여기서 데이터베이스의 상태를 별도로 업데이트하는 부분이 추가되어야 할 것이다.

우선 필자는 그정도까진 하진 않고 단순히 롤백만 하도록 하였으니 에러가 발생한다면 데이터베이스에는 상태가 `PROGRESS`가 아닌 `QUEUE`로 되면서도 작업은 유실될 것이기 때문에 실무에 사용하려면 이 부분은 별도의 핸들링이 필요하다는 점을 명심하자. 

<br />

![image](https://user-images.githubusercontent.com/71188307/147806906-543a7f3d-3c28-4009-af8e-916d23034138.png)
